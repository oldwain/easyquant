...
class MainEngine:
    ...
    def load_strategy(self, names=None, byfile=False):
        """动态加载策略
        :param names: 策略名列表，元素为策略的 name 属性
        :param byfile: True: 通过文件名进行过滤， False(defualt): 通过策略名进行过滤
        """

        s_folder = 'strategies'
        strategies = os.listdir(s_folder)
        if byfile:
            strategies = filter(lambda file: file.endswith('.py') and file.startswith('at')
                            and file != '__init__.py' and file[:-3] in names,
                            strategies)
        else:
            strategies = filter(lambda file: file.endswith('.py') and file.startswith('at') and file != '__init__.py',
                                strategies)
        ...
        for strategy_file in strategies:
            ...
            if byfile or names is None or strategy_class.name in names:
